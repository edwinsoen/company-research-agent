"""Headless verification runner for Phase 4 REST API & Agent Runtime.

Verifies:
1. POST /briefs starts Leg 1 execution and pauses at Gate 2 (approve_brief) with no active connection held.
2. GET /briefs/{id} retrieves current paused session status and draft payload.
3. POST /briefs/{id}/decision recovers pending call from session events, resumes Leg 2,
   and completes publishing.
4. Response returns status == 'completed' with valid published doc_url.
