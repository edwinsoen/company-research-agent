...
class MockToolContext:
    def __init__(self, state=None, memories=None):
...
class TestMemoryTools(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_memory_with_prior_brief(self):
        brief_data = {
            "company": "Stripe",
            "date": "2026-08-01",
        ...
        mem = MemoryEntry(
            ...
            custom_metadata={"topic": "company_brief_history", "company": "Stripe", "date": "2026-08-01"},
        )
    ...
    async def test_search_memory_scoped_isolation(self):
        ...
        mem = MemoryEntry(
            content=types.Content(role="user", parts=[types.Part.from_text(text=json.dumps(brief_data))]),
            custom_metadata={"topic": "company_brief_history", "company": "Stripe"},
        )
        ctx = MockToolContext(memories=[mem])

        # Querying for Acme must NOT return Stripe facts
